...
To reproduce and analyze the MoOsm1 structural models, a standardized PyMOL workflow was implemented to evaluate modeling confidence and conformational stability. Initially, the five models generated by AlphaFold 3 (ranked 0 to 4) were imported using the command `load fold_*.cif, Osm1_quarternarystate_PTMs_*`, where the asterisk represent each model. To streamline the manipulation of these structures, they were consolidated into a single functional group using `group quarternarystate_PTMs, Osm1_quarternarystate_PTMs_0 Osm1_quarternarystate_PTMs_1 Osm1_quarternarystate_PTMs_2 Osm1_quarternarystate_PTMs_3 Osm1_quarternarystate_PTMs_4`.

Structural convergence was assessed by superimposing models 1 through 4 onto the highest-confidence Model 0 using the command `align Osm1_quarternarystate_PTMs_*, Osm1_quarternarystate_PTMs_0`, followed by a focal adjustment with `zoom Osm1_quarternarystate_PTMs`. Local modeling reliability was visualized by applying a specific chromatic gradient based on the pLDDT (B-factor) scores in ascending order. Low-confidence regions were identified with `color orange, (quarternarystate_PTMs and b < 50)`, medium-confidence zones with `color yellow, (quarternarystate_PTMs and b > 50)`, and high-confidence regions with `color lightteal, (quarternarystate_PTMs and b > 70)`. Finally, very high-confidence structures were highlighted in dark blue using `color marine, (quarternarystate_PTMs and b > 90)`.
...
For the dual-phosphorylated monomer (@fig_P3_monomer_AFcolored_bothP), the alignment of models 1 through 4 against the top-ranked Model 0 demonstrated a mean RMSD of 0.37 Å.
...
The dimeric models (@fig_P3_dimer_AFcolored_171P) exhibited a bifurcated behavior in their convergence; while the first two subsequent models showed high structural similarity to Model 0 with a mean RMSD of 0.54 Å, the final two models displayed a drastic divergence, reaching an RMSD of nearly 30 Å.
...
@martix lo del rosetta server?  -> @@@carlota no hay rosetta server je
